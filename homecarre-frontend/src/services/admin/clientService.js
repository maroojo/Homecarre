...
const ClientService = () => {
  ...
  const getClients = async (page = 1, pageSize = 10) => {
    try {
      const response = await api(
        `${endpoint}/a/getclient?page=${page}&pagesize=${pageSize}`,
        "GET"
      );
      return response;
    } catch (error) {
  ...
  const getClient = async (keyword, dateRange, page = 1, pageSize = 10) => {
    ...
    try {
      const response = await api(
        (() => {
          let url = `${endpoint}/a/getclient?page=${page}&pagesize=${pageSize}`;
  ...
  const updateClient = async (clientData) => {
    try {
      const response = await api(`${endpoint}/a/updateclient`, "POST", {
  ...
  const insertClient = async (clientData) => {
    try {
      const response = await api(`${endpoint}/a/insertclient`, "POST", {
        fullname: clientData.fullname,
        client_phone: clientData.client_phone,
        client_type: clientData.client_type,
